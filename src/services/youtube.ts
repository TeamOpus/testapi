...
export class YouTube {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                const options: any = {
                    ...
                    addHeader: headers
                    // Removed problematic options: extractFlat, writeSubVtt, writeAutoSub
                };
                ...
                if (error.message?.includes('no such option')) {
                    console.error('🔧 Invalid yt-dlp option detected. Using minimal configuration...');
                    
                    // If this is due to invalid options, don't retry with the same config
                    if (attempt === maxRetries) {
                        console.error('💡 Try using basic options only or check yt-dlp documentation');
                    }
                } else if (error.message?.includes('Failed to extract any player response')) {
    ...
    static async getVideoInfo(url: string | any): Promise<VideoData | null> {
        try {
            ...
            if (error.message?.includes('no such option')) {
                console.error('🔧 SOLUTION: Invalid yt-dlp options removed. Please rebuild and try again.');
            } else if (error.message?.includes('Failed to extract any player response')) {
